...
function persisted(state: CortexState): CortexPersistedState {
  return {
    ...
    sessions: state.sessions.map((session) => ({
      ...session,
      status: "inactive",
      terminalHistory: state.settings.terminalHistoryEnabled
        ? trimTerminalHistory(session.terminalHistory)
        : "",
    })),
...
function normalizeLoadedState(state: CortexPersistedState): CortexPersistedState {
  ...
  return {
    ...
    sessions: (state.sessions ?? []).map((session) => ({
      ...
      status: "inactive",
      terminalHistory: loadedSettings.terminalHistoryEnabled
        ? trimTerminalHistory(session.terminalHistory ?? "")
        : "",
    })),
...
export const useCortexStore = create<CortexState>((set) => ({
  ...
  setFeatureFlag: (key, enabled) =>
    set((state) => {
      const next = {
        ...state,
        sessions:
          key === "terminalHistoryEnabled" && !enabled
            ? state.sessions.map((session) => ({ ...session, terminalHistory: "" }))
            : state.sessions,
        settings: {
          ...state.settings,
          [key]: enabled,
  ...
  clearAllTerminalHistory: () =>
    set((state) => {
      const next = {
        ...state,
        commandHistory: [],
        sessions: state.sessions.map((session) => ({ ...session, terminalHistory: "" })),
      };
      saveState(next);
      return next;
    }),
  ...
  importPersistedState: (importedState) =>
    set((state) => {
      const normalized = normalizeLoadedState(importedState);
      const next = { ...state, ...normalized, profiles, hydrated: true };
      saveState(next);
      return next;
    }),

  setUpdateCheckMode: (mode) =>
    set((state) => {
      const next = { ...state, settings: { ...state.settings, updateCheckMode: mode } };
      saveState(next);
      return next;
    }),
  ...
  appendTerminalHistory: (sessionId, output) =>
    set((state) => {
      if (!output || !state.settings.terminalHistoryEnabled) {
        return state;
      }
