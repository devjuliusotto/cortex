...
export function OfficeView({ externalWindow = false, onClose, onTerminalSelect }: OfficeViewProps) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col bg-background">
    ...
    <div className="relative min-h-0 flex-1 p-3"><OfficeStage agents={visibleAgents} workspaceName={officeName} showWorkspaceLabels={allProjects} summary={summary} cards={cards} git={git} onAgentSelect={(id) => { setBossOpen(false); setSelectedId(id); }} onTerminalOpen={openTerminal} onBossSelect={() => { setSelectedId(null); setBossOpen((open) => !open); }} />
      ...
      {bossOpen && <aside className="absolute inset-y-3 right-3 z-10 w-[min(340px,calc(100%-1.5rem))] rounded-md border border-border bg-card/95 p-4 shadow-xl backdrop-blur"><div className="flex items-start justify-between"><div><p className="text-sm font-semibold">Office overview</p><p className="text-xs text-muted-foreground">{officeName}</p></div><Button size="icon" variant="ghost" onClick={() => setBossOpen(false)}><X className="h-4 w-4" /></Button></div><div className="mt-5 grid grid-cols-2 gap-3 text-xs"><BossMetric label="Active agents" value={summary.total} /><BossMetric label="Errors" value={summary.errors} /><BossMetric label="Builds / tests" value={summary.buildsTests} /><BossMetric label="Office events" value={scopedEvents.length} /></div><div className="mt-4 rounded border border-border bg-background/50 p-3"><p className="text-[10px] uppercase text-muted-foreground">Last office event</p><p className="mt-1 font-mono text-xs">{lastEvent?.label ?? "Waiting for activity"}</p></div><a className="mt-5 block text-center text-[10px] text-muted-foreground hover:text-foreground" href="https://github.com/pixel-agents-hq/pixel-agents" target="_blank" rel="noreferrer">Inspired by Pixel Agents</a></aside>}
...
function readOfficeAgents(workspaceId: string | null, scope: OfficeScope) {
  if (!workspaceId) return [];
  const state = useCortexStore.getState();
  return createOfficeAgents(state.sessions, state.commandHistory, { scope, currentWorkspaceId: workspaceId, workspaces: state.workspaces });
}

function useOfficeAgents(workspaceId: string | null, scope: OfficeScope) {
  const [agents, setAgents] = useState<OfficeAgentModel[]>(() => readOfficeAgents(workspaceId, scope));
  const exitTimers = useRef(new Map<string, number>());
  const previousZones = useRef(new Map<string, string>());
  useEffect(() => {
    let timer: number | null = null;
    setAgents(readOfficeAgents(workspaceId, scope));
    const refresh = () => {
      timer = null;
      const next = readOfficeAgents(workspaceId, scope);
      const nextIds = new Set(next.map((agent) => agent.id));
      for (const agent of next) {
        ensureAgentSpawned(agent.id, agent.terminalName, agent.workspaceId);
        const previousZone = previousZones.current.get(agent.id);
        if (previousZone && previousZone !== agent.zone) addMoveEvent(agent.id, agent.workspaceId, agent.zone, agent.zone === "meetingRoom" ? `Joined meeting: ${agent.meetingLabel ?? "Syncing task"}` : previousZone === "meetingRoom" ? `Left meeting for ${agent.zone}` : `Moved to ${agent.zone}`, agent.zone === "meetingRoom" || previousZone === "meetingRoom" ? "meeting" : "move");
