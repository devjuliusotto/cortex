...
import { addMoveEvent, addOfficeEvent, getOfficeEvents, officeEventsForAgent, subscribeOfficeEvents } from "./officeEvents";
import { AGENT_EXIT_DELAY_MS, officeTarget } from "./officeLayout";
import { detectOfficeAiAgents } from "./adapters/officeAgentAdapters";
...
export function OfficeView({ externalWindow = false, onClose, onTerminalSelect }: OfficeViewProps) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col bg-background">
    ...
    <div className="relative min-h-0 flex-1 p-3"><OfficeStage agents={visibleAgents} workspaceName={officeName} showWorkspaceLabels={allProjects} summary={summary} cards={cards} git={git} onAgentSelect={(id) => { setBossOpen(false); setSelectedId(id); }} onTerminalOpen={openTerminal} onBossSelect={() => { setSelectedId(null); setBossOpen((open) => !open); }} />
      {selected && <OfficeTimelinePanel agent={selected} events={officeEventsForAgent(selected.id)} onClose={() => setSelectedId(null)} onOpenTerminal={selected.terminalId ? () => openTerminal(selected.terminalId as string) : undefined} />}
...
async function readOfficeAgents(workspaceId: string | null, scope: OfficeScope) {
  if (!workspaceId) return [];
  const state = useCortexStore.getState();
  const aiAgents = await detectOfficeAiAgents({ scope, currentWorkspaceId: workspaceId, workspaces: state.workspaces, sessions: state.sessions, commandHistory: state.commandHistory });
  return createOfficeActors(aiAgents, state.sessions);
}
...
function useOfficeAgents(workspaceId: string | null, scope: OfficeScope) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      timer = null;
      const next = await readOfficeAgents(workspaceId, scope);
      if (disposed) return;
      const nextIds = new Set(next.map((agent) => agent.id));
      for (const agent of next) {
        if (!previousZones.current.has(agent.id)) addOfficeEvent({ agentId: agent.id, terminalId: agent.terminalId, workspaceId: agent.workspaceId, type: "spawn", label: `${agent.name} entered office`, zone: "entrance" });
        ...
        if (previousActivity.current.get(agent.id) !== agent.activity) {
          addOfficeEvent({ agentId: agent.id, terminalId: agent.terminalId, workspaceId: agent.workspaceId, type: agent.signal === "warning" ? "error" : "activity", label: agent.activity, detail: agent.toolName, zone: agent.zone });
