...
type Props = { agents: OfficeAgentModel[]; workspaceName: string; summary: OfficeSummary; cards: OfficeKanbanCard[]; git: OfficeGitSummary; onAgentSelect: (id: string) => void; onTerminalOpen: (id: string) => void; onBossSelect: () => void };

export function OfficeStage({ agents, workspaceName, summary, cards, git, onAgentSelect, onTerminalOpen, onBossSelect }: Props) {
  ...
  const bossLines = [`${summary.active} ACTIVE · ${summary.errors} ERR · ${summary.buildsTests} B/T`, workspaceName, summary.lastActivity];
  return <div ref={hostRef} className="h-full min-h-[420px] w-full overflow-hidden rounded-md border border-border bg-[#080b12] shadow-inner"><Application resizeTo={hostRef} backgroundAlpha={0} antialias={false} resolution={1}><pixiContainer x={sceneX} y={sceneY} scale={scale}>
    ...
    {agents.map((agent) => <OfficeAgent key={agent.id} agent={agent} onSelect={onAgentSelect} onOpen={onTerminalOpen} />)}
