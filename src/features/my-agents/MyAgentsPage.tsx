...
import {
  ...
  PackagePlus, Play, Plus, RefreshCw, Search, ShieldAlert, Sparkles, TerminalSquare,
} from "lucide-react";
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col overflow-hidden border-l border-border bg-cortex-graphite">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-6">
        ...
        {section === "skills" && <SkillsSection onCommand={openCommand} workspacePath={activeWorkspace?.defaultWorkingDirectory} />}
...
function SkillsSection({ onCommand, workspacePath }: { onCommand: (command: string) => void; workspacePath?: string }) {
  ...
  return <div className="mx-auto max-w-6xl space-y-5"><SectionHeading icon={<Sparkles className="h-5 w-5 text-primary" />} title="Agent Skills" text={skillsExplanation.trim()} />{!workspacePath && <div className="flex gap-2 rounded-md border border-cortex-amber/30 bg-cortex-amber/5 p-3 text-xs text-muted-foreground"><AlertTriangle className="h-4 w-4 shrink-0 text-cortex-amber" />Set a default terminal path on the active workspace before installing skills from the catalog.</div>}<section className="rounded-lg border border-border bg-card/65 p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><h3 className="text-sm font-semibold">Curated GitHub skills</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">30 popular skills from the public skills.sh leaderboard. Install commands open in a visible terminal for review.</p></div><label className="flex h-10 min-w-0 items-center gap-2 rounded-md border border-border bg-background px-3 text-xs text-muted-foreground lg:w-80"><Search className="h-4 w-4 shrink-0" /><input className="min-w-0 flex-1 bg-transparent text-sm text-foreground outline-none" onChange={(event) => setQuery(event.target.value)} placeholder="Search skills" value={query} /></label></div><div className="mt-4 flex gap-1 overflow-x-auto pb-1">{curatedSkillCategories.map((item) => <button className={cn("shrink-0 rounded-md px-2.5 py-1.5 text-xs text-muted-foreground hover:bg-secondary hover:text-foreground", category === item && "bg-secondary text-foreground")} key={item} onClick={() => setCategory(item)} type="button">{item}</button>)}</div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{filteredSkills.map((skill) => <CuratedSkillCard key={`${skill.repository}:${skill.id}`} onInstall={() => onCommand(skillInstallCommand(skill))} skill={skill} />)}</div>{filteredSkills.length === 0 && <div className="rounded-md border border-dashed border-border p-6 text-center text-sm text-muted-foreground">No skills match this search.</div>}</section><section><h3 className="mb-3 text-sm font-semibold">Skill folders</h3><div className="grid gap-3 md:grid-cols-2">{skillPaths.map((skill) => { const info = directories[skill.path]; const localUnavailable = skill.type === "local" && !workspacePath; return <article className="rounded-lg border border-border bg-card/65 p-4" key={skill.path}><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-semibold">{skill.type === "global" ? "Global" : "Workspace"} skill path</div><code className="mt-1 block text-xs text-primary">{skill.path}</code></div>{info?.exists ? <CheckCircle2 className="h-5 w-5 text-cortex-green" /> : <Plus className="h-5 w-5 text-muted-foreground" />}</div><p className="mt-3 text-xs leading-5 text-muted-foreground">{skill.description}</p>{info && <div className="mt-2 break-all font-mono text-[10px] text-muted-foreground">{info.path}</div>}<div className="mt-4 flex gap-2"><Button size="sm" variant="outline" disabled={localUnavailable || !info?.exists} onClick={() => void invoke("open_skill_directory", { path: skill.path, workspacePath })}><FolderOpen className="mr-2 h-4 w-4" />Open folder</Button>{!info?.exists && <Button size="sm" disabled={localUnavailable} onClick={() => void createDirectory(skill)}><Plus className="mr-2 h-4 w-4" />Create folder</Button>}</div></article>; })}</div></section></div>;
}
