...
fn transcript_snapshot(
    ...
    let modified_at = metadata.modified().ok()?.duration_since(UNIX_EPOCH).ok()?.as_millis() as u64;
...
#[tauri::command]
fn office_read_claude_transcripts(workspace_paths: Vec<OfficeWorkspacePath>) -> Result<Vec<ClaudeTranscriptSnapshot>, String> {
    ...
    let canonical_root = projects_root.canonicalize().map_err(|error| error.to_string())?;
    ...
    for workspace in workspace_paths {
        ...
        let Some(project_dir) = project_entries.iter().find(|entry| entry.file_name().to_string_lossy().eq_ignore_ascii_case(&expected)).and_then(|entry| entry.path().canonicalize().ok()).filter(|path| path.starts_with(&canonical_root)) else {
            continue;
        };
        ...
        for entry in entries.filter_map(Result::ok) {
            ...
            if path.extension().and_then(|value| value.to_str()) == Some("jsonl") {
                if let Some(snapshot) = transcript_snapshot(&workspace.workspace_id, &path, false, None) { snapshots.push(snapshot); }
                continue;
            }
            if !path.is_dir() { continue; }
            let parent_session_id = path.file_name().map(|value| value.to_string_lossy().to_string());
            let subagents_dir = path.join("subagents");
            let Ok(subagents) = fs::read_dir(subagents_dir) else { continue };
            for subagent in subagents.filter_map(Result::ok) {
                ...
                if subagent_path.extension().and_then(|value| value.to_str()) == Some("jsonl") {
                    if let Some(snapshot) = transcript_snapshot(&workspace.workspace_id, &subagent_path, true, parent_session_id.clone()) { snapshots.push(snapshot); }
                }
            }
        }
    }
